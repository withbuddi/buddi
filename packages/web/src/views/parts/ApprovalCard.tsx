...
export function useDecide(onDone: () => void): {
  ...
  note: string | null;
  failure: string | null;
  decide: (id: string, decision: Decision, scope?: Scope) => Promise<void>;
} {
  const [busy, setBusy] = useState<string | null>(null);
  const [note, setNote] = useState<string | null>(null);
  const [failure, setFailure] = useState<string | null>(null);
  const decide = async (id: string, decision: Decision, scope?: Scope): Promise<void> => {
    setBusy(id);
    setFailure(null);
    setNote(null);
    try {
...
export function ApprovalCard({
  ...
  timezone: string;
  busy: boolean;
  onDecide: (id: string, decision: Decision, scope?: Scope) => void;
  agentName?: string;
}): JSX.Element {
