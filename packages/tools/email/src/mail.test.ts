...
describe('the owner\'s own addresses', () => {
  it('collapses case, plus-tags and Gmail\'s dots onto one mailbox', () => {
    expect(mailboxKey('Amouzou@Gmail.COM')).toBe('amouzou@gmail.com');
    expect(mailboxKey('amouzou+cdc@gmail.com')).toBe('amouzou@gmail.com');
    expect(mailboxKey('a.mouzou@googlemail.com')).toBe('amouzou@gmail.com');
  ...
  it('recognises the same mailbox however it is written', () => {
    expect(isSameMailbox('amouzou+x@gmail.com', 'A.Mouzou@googlemail.com')).toBe(true);
...
describe('who a reply goes to', () => {
  const original = {
    ...
    to: ['amouzou@gmail.com', 'rachid@example.test', 'fiatyao@example.test'],
    cc: ['successor@example.test', 'Amouzou+cdc@gmail.com'],
    owner: ['amouzou@gmail.com'],
  };
  ...
  it("never puts the owner on a reply, in any form of his address", () => {
    ...
    expect(everyone).not.toContain('amouzou@gmail.com');
    expect(everyone).not.toContain('Amouzou+cdc@gmail.com');
    expect(everyone.some((a) => mailboxKey(a) === 'amouzou@gmail.com')).toBe(false);
    expect(reply.excludedOwn).toEqual(['amouzou@gmail.com', 'amouzou+cdc@gmail.com']);
  });
  ...
  it('adds named people without changing the shape', () => {
    const reply = replyRecipients({
      ...
      alsoCc: ['watcher@example.test', 'amouzou@gmail.com'],
    });
  ...
  it('has nobody to reply to when the owner is the only address on it', () => {
    const reply = replyRecipients({
      from: 'amouzou+self@gmail.com',
      to: ['amouzou@gmail.com'],
      owner: ['amouzou@gmail.com'],
      audience: 'everyone',
    });
    expect(reply.to).toEqual([]);
  ...
  it('counts nobody twice, and never the owner or the sender, among the others', () => {
    const reply = replyRecipients({
      from: 'her@example.test',
      to: ['HER@example.test', 'amouzou+cdc@gmail.com', 'him@example.test'],
      cc: ['him@example.test', 'a.mouzou@googlemail.com'],
      owner: ['amouzou@gmail.com'],
    });
