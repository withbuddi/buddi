...
export const search: ToolDefinition<z.infer<typeof searchInput>, unknown> = {
  ...
  untrusted: 'mail',
  description:
    'Search ingested mail. Give `query` for a case-insensitive substring of the subject, the sender or the body, and narrow it with any of `from` (a whole address, or a bare domain which also matches its subdomains), `since` and `until` (YYYY-MM-DD, against when the mail was received, read in the owner\'s own timezone), `thread` (one conversation), `direction` (`in` or `out`), and `hasAttachments`. `query` may be left out when at least one filter is given — `from: "acme.com", hasAttachments: true` is a search. Newest first, across every mailbox unless you name one with `account`. Subjects and senders are always searched in full; with a `query` and nothing to bound it (`since`, `from` or `thread`) only the last 90 days of message *bodies* are read, and the answer says so. Use it to find the earlier message a new one refers to. For a one-time code a site just sent, give `from` (the site\'s domain) and `since` (today) and take the newest message dated in the last ten minutes; new mail is fetched every five minutes, so search again shortly if it is not there yet.',
  tier: 'auto',
  input: searchInput,
  async execute(input, ctx) {
    const scope = await accountScope(ctx.buddi!.db, input.account);
