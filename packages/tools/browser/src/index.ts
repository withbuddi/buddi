...
export function hostBrowser(env: NodeJS.ProcessEnv = process.env): HostController {
    ...
    tools: [
      ...
      { name: 'browser.act', tier: 'session', sequential: true, input: commandSchema,
        ...
        image: async (output, ctx) => {
          ...
          return bytes ? { mime: 'image/jpeg', data: bytes.toString('base64') } : undefined;
        },
      },
    ],
...
export { commandSchema, UNTRUSTED } from './types.js';
export type { BrowserCommand, BrowserDriver, Observation } from './types.js';
