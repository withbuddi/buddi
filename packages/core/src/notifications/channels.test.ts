...
describe('channels', () => {
  ...
  it('defaults to the lowest priority, then the first registered', () => {
    add('email.self');
    expect(channelFor(SETTINGS)).toBe('email.self');
    add('local.notification', 10);
    add('other.thing', 10);
    expect(channelFor(SETTINGS)).toBe('local.notification');
    add('telegram.chat', 0);
    expect(channelFor(SETTINGS)).toBe('telegram.chat');
    expect(listChannels().map((c) => c.kind)).toEqual(['telegram.chat', 'local.notification', 'other.thing', 'email.self']);
    expect(channelFor({ ...SETTINGS, defaultChannel: 'email.self' })).toBe('email.self');
  });
