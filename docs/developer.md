---
title: "The developer plugin: an agent that works in a workspace"
status: reference
updated: 2026-09-25
---
...
The `developer` plugin lives in the buddi-plugins repository. It gives one
agent a directory to read, edit, run and commit in, with a mode the owner
chooses once. Its own `buddi.md` is what the owner reads at install; this page
is the design.
...
exist outside it. Host execution set to "Always allowed" is the crude
version of the same wish; this is the bounded one.
...
runs. `~/.ssh`, `~/.aws`, `~/.gnupg`, `~/.npmrc`, `~/.netrc`, `~/.gitconfig`,
`~/.config/gh`, the owner's shell rc files, `~/Library/Keychains`, `~/.buddi`,
buddi's own data directory and the vault are refused even when a workspace is
placed above them.
...
A workspace can be a folder that holds several projects. Reads, edits,
searches and the run list are about paths, not repositories, so they work
there. Two things follow, and the agent is told both: the git tools refuse,
because the workspace is not a repository root (§4); and symbolic links are
stepped over when listing and searching, with the result saying how many
entries were skipped. Only a link in the path the agent *asked for* is a
refusal, since then there is nothing to list.

## 3. Modes, chosen once

The mode is a per-agent setting, changed by the owner on Settings → Developer
(`developer.set_mode`, owner-only), never by the agent:
...
- `run`: a short, pinned list of commands runs inside the workspace without a
  ...
  project's own scripts then run as them. **The default**: `edit` as the
  default would make every first session a wall of cards for `npm run build`,
  and an owner who granted a workspace at all almost always wants this one.
  The other two are a select away.
...
the record on every call, so changing it on Settings → Developer changes the
next call and not the run.
...
`developer.run` even in `run` mode (§10, item 5), a scheduled run with no
...
- `developer.screenshot` `{ name, path?, port? }`: a picture of a page one of
  the agent's running processes serves. A throwaway headless browser loads
  `http://127.0.0.1:<port><path>` and nothing else; the port must be one the
  process is listening on, and the PNG goes to the owner's Files library and
  onto the canvas.
...
each with a Forget action (`developer.forget_command`, owner-only). The mode
of each agent is set there (`developer.set_mode`), "Stop all processes" is
`developer.stop_all`, and the Tailscale switch (§12) is
`developer.set_settings`; all four are `ownerOnly`, so no model is shown them.
...
## 11. What it stores, and what runs on a timer

Schema `developer`:

- `workspaces` — one row per agent: the directory, the mode (`ask`, `edit`
  or `run`), the captured `PATH` and the git binary on it.
...
File contents are read and handed to the run that asked for them; they are not
stored. Outside the database it writes one log file per running process, at
`developer/logs/<agent>.<name>.log` under buddi's data directory, and one empty
directory, `developer/empty-hooks`, that git's `core.hooksPath` points at.

Nothing runs on a timer: the plugin has no sources and no sentinels. It
declares no network hosts. It proposes one agent, `developer` (handle `@dev`),
granted this plugin's tools and nothing else, with one skill — the loop it
works in, what the mode decides, and the branch rule.

One metric a goal can watch ([goals.md](goals.md)): `developer.failing_tests`,
how many tests were failing in the last test run recorded for the calling
agent's workspace, the same run `developer.summarise` reports. It should go
down. Only a real test runner is believed — `vitest`, `jest`, `pytest`,
`python -m pytest|unittest`, `go test`, `cargo test`, `npm|pnpm|yarn|bun test`,
`node --test`, `make test` — because a green typecheck measured as "0 failing
tests" would settle a goal as met with nothing tested. The count is read from
the runners' own summary lines and added up across them. With no workspace, no
recorded run (a restart loses it), a run that was not a test runner, or a
failed run whose output never said how many tests failed, the check records
*not measurable* rather than a zero.
...
- **Through buddi.** The gateway serves `/preview/<plugin>/<process>/…` on a
  **second listener** of its own (the dashboard port plus one, or
  ...
  offered; it would be a signed, expiring variant of the same link.

  In detail (docs/plugins.md §2.5c): the plugin declares
