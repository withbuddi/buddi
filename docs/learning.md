---
title: "Learning: buddi proposes, the owner keeps"
status: reference
updated: 2026-09-25
---
...
An agent that does the same thing twice should not have to work it out twice.
This page is how buddi learns from what its agents do, and why nothing it
learns applies itself.

## 1. The rule

An agent that rewrites its own instructions with nobody reading is not
improving, it is drifting, and a page that says "from now on always do X" must
never become part of an agent. So:
...
1. **Memory**: facts and preferences, kept by the memory plugin. The digest
   (§5) lists what was remembered this week.
2. **Skills**: a procedure learned from experience. After a task that took
   ...
   is a diff. The agent's Skills tab lists what it learned, with version and
   provenance.
3. **Policies**: a repeated decision becomes a rule. An agent calls
   `learning.propose_policy`, and a plugin `ctx.buddi.proposals.proposePolicy`
   ([plugin-host-api.md](plugin-host-api.md) §4.2), with the plugin, the
   matcher, the action and the verdicts it was learned from; core records the
   proposal, and the plugin applies kept policies itself. The email
   plugin's learned ignore and notify rules take this path
   ([email.md](email.md) §3).
4. **Persona and tools**: an agent may propose a change to its own
   ...
   shown as a diff of its agent file; Agent Father's update flow applies
...
A proposal with any source is marked "made with untrusted text in view" on
the card, and a skill proposed in such a run is shown with the sentences that
came from those sources highlighted, so an instruction smuggled in a page is
visible before it is kept. A proposal made in a run that had no untrusted
input is unmarked.
...
The system context carries one paragraph: when a task took several steps and
