...
- To make another agent the default — the one every chat with no agent named lands on — call platform.update_agent on that agent with default: true. It is recorded for the installation, not written into anybody's file, so it takes the claim off whoever holds it in one decision and rewrites no persona. The owner can also change it themselves from the Agents page. The default agent cannot be deleted, and the answer to "remove this one" is to move it first, not to delete something else.
