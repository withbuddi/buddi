...
import { nativeEnvironment } from './environment.mjs';
...
export async function binaries(ctx) {
  ...
  let entry;
  try { entry = ctx.require.resolve(name); }
  ...
  if (!existsSync(path.join(dest, '.ready'))) {
    ...
    const links = JSON.parse(await readFile(path.join(native, 'pg-symlinks.json'), 'utf8').catch(() => '[]'));
    // Upstream's source is the link destination; target is the link to create.
    for (const { source, target } of links) {
      const rebase = value => {
        const normalized = value.replaceAll('\\', '/');
        const suffix = normalized.includes('/native/') ? normalized.split('/native/').pop() : normalized.replace(/^native\//, '');
...
export async function stopChild(child, signal = 'SIGTERM', timeout = 15_000) {
...
export function watchDatabase(probe, { interval = 1000, failures = 3 } = {}) {
  let timer, stopped = false, alive = true, misses = 0, finish;
...
async function initializeCluster(bin, stage, password, env) {
...
export async function startDatabase(ctx, core) {
  if (ctx.state.database === 'external') {
    ...
    return { stop: async () => {}, pid: null, alive: true, exited: new Promise(() => {}) };
  }
  ...
  try {
    for (;;) {
      ...
      try {
        ...
        const sql = await client.query("SELECT format('ALTER ROLE %I LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE PASSWORD %L', $1::text, $2::text) AS statement", ['buddi', password]);
        ...
        if (error.message === 'database-port-conflict') throw new Error(`Database port ${ctx.state.dbPort} belongs to a different cluster. No roles or databases on it were changed.`);
  ...
  return { pid: child?.pid ?? adoptedPid, child, exited: monitor.exited, get alive() { return monitor.alive; }, stop: async () => {
    ...
    if (child) await stopChild(child, 'SIGINT');
      ...
      catch (error) { if (error.code === 3) running = false; else throw error; }
